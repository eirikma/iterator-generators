...
public final class Iterators {
    ...
    /**
     * Creates a generator, generating objects on the fly while some other code iterates the created objects.
     * A generator is a lambda function called whenever the iterator needs a new object. The function must
     * 'yield' objects  (using the GeneratorState.yield(object) at least once for every iteration, otherwise
     * the iterator's hasNext() method will return false. yielding more than one value is okay. The generator helper code
     * will ensure these are delivered to/through the iterator in the order they were yielded.
     *
     * @param g the object generating algorithm, that must call state.yield(object) at least once for each invocation, otherwise the iterator's hasNext() returns false.
     * @param <T> the type of elements created/delivered/generated by this generator
     * @return the output of the generator represented as an repeatable iterator. The repeatability depends upon the generator code's ability to re-initialize.
     */
    public static <T> RepeatableIterator<T> generator(Generator<T, Void> g) {
        return new GeneratorImpl<T, Void>(g);
    }

    /**
     * Creates a generator where a user-defined Context-object is made available on each invocation of the generator closure.
     * The object is made available through the GeneratorState.context() method.
     *
     * @param context - a user-defined context object that will be available inside the generator method.
     * @param g - the generator function
     * @param <T> - the type of elements generated by the generator
     * @param <C> - the class of the user-defined context method.
     * @return an iterator interacting with the generator, so the output can be iterated over (usually generated when the hasNext() method is called).
     */
    public static <T,C> RepeatableIterator<T> generator(C context, Generator<T,C> g) {
        return new GeneratorImpl<T, C>(g, context);
    }


    /**
     * Generates an inputProcessor, which is a generator operating on some input from an iterator.
     * The iterator will be made available as an pushBackIterator, to handle complex input scenarios where
     * you don't know the exact end of the input used for yielding one element of output (i.e. when parsing something complex).
     *
     * @param input  -the input
     * @param processor - the generator code.
     * @param <I> the type of the input
     * @param <O> the type of the yielded output
     * @return an iterator that will invoke the processor whenever the iterating code is requesting new objects.
     */
    public static <I,O> Iterator<O> inputProcessor(Iterator<I> input, InputProcessor<I,O, Void> processor) {
        return new InputProcessorImpl<Void, I, O>(pushbackable(input), processor);
    }
     ...
     * @return an iterator that will invoke the processor whenever the iterating code is requesting new objects.
     */
    public static <C,I,O> Iterator<O> inputProcessor(C context, Iterator<I> input, InputProcessor<I,O,C> processor) {
        return new InputProcessorImpl<C, I, O>(context, pushbackable(input), processor);
    }
